...
```
https://www.youtube.com/watch?v=G9LyaVJovu8
...
```{r echo=FALSE}
blogdown::shortcode_html('gfycat', 'leanunevenbull')
```
...
```{r echo=FALSE}
blogdown::shortcode_html('gfycat', 'untriedsphericalamazondolphin')
```
...
```{r echo=FALSE}
blogdown::shortcode_html('gfycat', 'SmugSpiffyDegu')
```
...
```{r echo=FALSE}
blogdown::shortcode_html('gfycat', 'AgedDimpledAmethystinepython')
```
...
```{r echo=FALSE}
blogdown::shortcode_html('gfycat', 'AgreeableAncientConure')
```
...
```{r echo=FALSE}
blogdown::shortcode_html('gfycat', 'bogussharpizuthrush')
```
Nice huck shape from Penn. Check footwork

```{r echo=FALSE}
blogdown::shortcode_html('gfycat', 'distortedjealousblackrhino')
```
...
```{r echo=FALSE}
blogdown::shortcode_html('gfycat', 'remotesilentisabellineshrike')
```
...
```{r echo=FALSE}
blogdown::shortcode_html('gfycat', 'illopulentguineapig')
```
...
```{r echo=FALSE}
blogdown::shortcode_html('gfycat', 'talkativespecificarabianwildcat')
```
...
```{r echo=FALSE}
blogdown::shortcode_html('gfycat', 'WeeklyUltimateAmethystgemclam')
```
Marking in isolation. Disc is on break sideline - we should prioritize taking away low throws here. Higher throws almost by definition have more time to get d’ed. Also, should you shade inside, then transition around? Hard to say, seems like the around could be floated out there. 
Maybe it’s the classic left-hand-low-right-hand-high scenario. Defender gets caught the fuck out looking though, great opportunity by Texas cutter. Not sure if they knew it was a bracket scenario. Also curious if marker should be at the 2 o’clock (peripheral upfield) position. 
Should mark be close or far? I actually think this throw would have been taken away if the marker had been two steps back.
...
```{r echo=FALSE}
blogdown::shortcode_html('gfycat', 'frequentenragedbelugawhale')
```
